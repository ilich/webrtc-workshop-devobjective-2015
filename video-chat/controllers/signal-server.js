function signalServer(expressServer) {
	...
	io.on('connection', function (socket) {
		...
		socket.on('send-file', function (fileInfo) {
			console.log('%s: file transfer %s (%s bytes)', socket.room, fileInfo.name, fileInfo.size);
			socket.to(socket.room).emit('send-file', fileInfo);
		});

		socket.on('send-file-accepted', function () {
			console.log('%s: file transfer confirmed', socket.room);
			socket.to(socket.room).emit('send-file-accepted');
		});
